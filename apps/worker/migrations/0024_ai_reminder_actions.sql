-- Expand the AI action allowlist for reminder completion without rewriting 0023.
...
ALTER TABLE ai_email_outbox RENAME TO ai_email_outbox_0023;
ALTER TABLE ai_action_proposals RENAME TO ai_action_proposals_0023;

CREATE TABLE ai_action_proposals (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  tool TEXT NOT NULL CHECK (tool IN ('create_note','create_reminder','complete_reminder','create_notification','send_email','update_note','move_note','archive_note','restore_note','delete_note','create_folder','apply_tag','create_database_record','update_database_record','apply_template')),
...
SELECT id, action_id, user_id, workspace_id, to_email, subject, body_text,
  status, attempt_count, available_at, sent_at, last_error_code, created_at, updated_at,
  dispatch_lease_until, dispatch_claim_token, delivery_lease_until, delivery_claim_token
FROM ai_email_outbox_0023;

DROP TABLE ai_email_outbox_0023;
DROP TABLE ai_action_proposals_0023;
