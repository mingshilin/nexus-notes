...
describe("AI chat proxy", () => {
  it("registers an authenticated workspace route with viewer access", () => {
    ...
    expect(definitions.map(({ method, path, auth }) => ({ method, path, auth }))).toEqual([
      ...
      { method: "GET", path: "/api/v2/ai/trusted-mode", auth: "workspace" },
      { method: "PATCH", path: "/api/v2/ai/trusted-mode", auth: "workspace" },
      { method: "GET", path: "/api/v2/ai/actions/history", auth: "workspace" },
