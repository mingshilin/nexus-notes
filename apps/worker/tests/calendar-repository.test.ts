...
describe("D1CalendarRepository", () => {
  ...
  it("does not resurrect a revoked connection when a sync write races with disconnect", async () => {
    const test = await createTestD1();
    try {
      await seed(test.db);
      const repository = new D1CalendarRepository(test.db, { createId: (() => { let n = 0; return () => `id-${++n}`; })() });
      await repository.upsertConnection({ id: "connection-1", userId: "user-1", provider: "google", providerAccountId: "account-1", encryptedRefreshToken: encrypted, now });
      await repository.revokeConnection("user-1", "connection-1", now);
      await expect(repository.upsertEvents("user-1", "connection-1", [{
        id: "temporary", connection_id: "connection-1", provider: "google", provider_event_id: "same-event",
        title: "Must not return", starts_at: "2026-08-28T01:00:00.000Z", ends_at: "2026-08-28T02:00:00.000Z", timezone: "UTC", all_day: false, status: "confirmed", updated_at: now,
      }])).resolves.toBe(false);
      await expect(repository.markSync("user-1", "connection-1", { status: "active", syncCursor: "cursor", syncFrom: "2026-08-28", syncTo: "2026-08-28", lastSyncedAt: now, lastErrorCode: null, now })).resolves.toBeNull();
      await expect(repository.listEvents("user-1", { from: "2026-08-28", to: "2026-08-28" })).resolves.toEqual([]);
    } finally {
      await test.dispose();
    }
  });

  it("gives identical provider event IDs different row IDs across connections", async () => {
    const test = await createTestD1();
    try {
      await seed(test.db);
      const repository = new D1CalendarRepository(test.db, { createId: (() => { let n = 0; return () => `connection-${++n}`; })() });
      await repository.upsertConnection({ id: "connection-1", userId: "user-1", provider: "google", providerAccountId: "account-1", encryptedRefreshToken: encrypted, now });
      await repository.upsertConnection({ id: "connection-2", userId: "user-1", provider: "google", providerAccountId: "account-2", encryptedRefreshToken: encrypted, now });
      const item = { id: "temporary", connection_id: "connection-1", provider: "google" as const, provider_event_id: "same-event", title: "Same", starts_at: "2026-08-28T01:00:00.000Z", ends_at: "2026-08-28T02:00:00.000Z", timezone: "UTC", all_day: false, status: "confirmed" as const, updated_at: now };
      await repository.upsertEvents("user-1", "connection-1", [item]);
      await repository.upsertEvents("user-1", "connection-2", [{ ...item, connection_id: "connection-2" }]);
      const rows = await test.db.prepare("SELECT id FROM calendar_events ORDER BY connection_id").all<{ id: string }>();
