...
import { RECORD_COLUMNS, DatabaseRepositoryError, placeholders, type RecordRow } from "./database-model";
...
export class D1DatabaseRecordRepository extends DatabaseRepositoryBase {
  ...
  async searchRecords(context: WorkspaceContext, databaseId: string, options: { query: string; limit: number }) {
    const fields = await this.access.fields(context, databaseId, "read");
    ...
    const result = await this.db.prepare(
      `SELECT DISTINCT r.${RECORD_COLUMNS.split(", ").join(", r.")}
       ...
       WHERE r.workspace_id = ? AND r.database_id = ? AND r.deleted_at IS NULL
         AND v.property_id IN (${placeholders(propertyIds.length)}) AND v.value_json LIKE ? ESCAPE '\\'
       ORDER BY r.updated_at DESC, r.id DESC LIMIT ?`,
    ).bind(
      context.workspaceId, databaseId, ...propertyIds,
      `%${options.query.replaceAll("%", "\\%").replaceAll("_", "\\_")}%`, Math.min(options.limit, 100),
    ).all<RecordRow>();
    return { items: await this.materialize(result.results ?? [], fields.readable), next_cursor: null };
  }
  ...
  async bulkEditRecords(context: WorkspaceContext, databaseId: string, input: BulkEditRecordsInput) {
    ...
    const prepared = input.mutations.map((mutation) => {
      ...
      return { mutation, values: this.normalize(fields.properties, mutation.values, fields.writable) };
    });
    const now = this.now();
