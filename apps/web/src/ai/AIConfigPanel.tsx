import type { AiUserConfigSummary } from "@nexus/contracts";
...
export function AIConfigPanel({ client }: Props) {
  const [summary, setSummary] = useState<AiUserConfigSummary | null>(null);
  const [baseUrl, setBaseUrl] = useState("");
  const [model, setModel] = useState("");
  const [apiKey, setApiKey] = useState("");
  const [pending, setPending] = useState<"test" | "save" | "delete" | null>(null);
