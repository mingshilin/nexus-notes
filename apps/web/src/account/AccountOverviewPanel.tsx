...
export function AccountOverviewPanel({ client, active, onOpenAiControls }: { client: ProfileClientLike; active: boolean; onOpenAiControls?(): void }) {
  ...
  return (
    <section id="account-panel-overview" role="tabpanel" aria-labelledby="account-tab-overview" className="account-panel">
      ...
      {error ? <div className="account-error-row"><p role="alert" className="account-error">{error}</p><button type="button" onClick={() => setRetry((value) => value + 1)}>重试账户总览</button></div> : null}
      {overview ? <>
        <div className="account-overview-grid">
          ...
          <article><strong>{overview.ai_configured ? "AI 已配置" : "AI 未配置"}</strong><span>可在 AI 助手中管理个人 provider</span>{onOpenAiControls ? <button type="button" onClick={onOpenAiControls}>查看 AI 控制</button> : null}</article>
        </div>
