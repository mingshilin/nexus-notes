...
describe("database web state", () => {
  it("persists pagination independently by workspace, database, and view", async () => {
    ...
    store.write("ws-1", "db-1", "view-table", { page: 3, pageSize: 25, cursor: "cursor-3" });
    store.write("ws-1", "db-1", "view-board", { page: 1, pageSize: 50, cursor: null });

    const restored = new data.DatabasePaginationStore(storage);
    expect(restored.read("ws-1", "db-1", "view-table")).toEqual({ page: 3, pageSize: 25, cursor: "cursor-3" });
    expect(restored.read("ws-1", "db-1", "view-board")).toEqual({ page: 1, pageSize: 50, cursor: null });
